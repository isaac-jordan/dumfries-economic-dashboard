...
Initially the plan was to have a number of different types of graphs, however, the data which was acquired was mainly timeline based. Therefore the team decided to mainly implement line graphs since they display that type of data most effectively. A few sample bar graphs were added later on for demonstration purposes. Potentially many more types of graphs could have been added, however, the team decided to invest that time on higher priority issues.

One major issue raised by the client was the ease of use of the website. Since this website was meant to be used by people with different skillsets the team decided to add helpful guiding documentation for both the users and the administrators. On the left bar on the main page of the website a “Help/Documentation” link was added which redirects the user to a clear and easy to understand tutorial explaining all the features of the website and how to use them. Analogically on the admin panel two link were added to guide the administrators if they wanted to change something on the dashboard or add a new CSV file through the CSV processor.

One of the last major issues, addressed by the team ,was the visualization of the
Dashboard. The progress of the project reached to a point where the dashboard
provided almost all the required functionalities, the team decided to give an emphasis
on the visualisation in order to make the dashboard easy to use and user friendly.
D3 is a framework that provides many different functions to the developer , it allows
to work on the flexibility of the graphs and the visualisation on the same time. A
tooltip was created to show the exact coordinates their value and their label as long
as the mouse cursor pass over a line of the graph. For the bar graphs, due to the
reason that they were used for demonstration , a simple change of colour , each time cursor was passing over a bar. was implemented. It was ensured also in case of 1 element a large circle to be created with text (X axis label).

While discussing the progress of the project with the client at one of the later meetings it was pointed out that people with visual impairments such as colour blindness or problematic eyesight may have trouble using the website, however, the client made it clear that the team should look into that only if time allows it and to mark it as a low priority. The team decided to invest some time into researching and experimenting with different colour styles and word formats. Two different styles were added that changed the overall colour and the overall word size, however, due to time restrains and more urgent issues at hand, this feature was left semi-completed. 
...
%We talk about how the team appointed a customer liaison to communicate with and receive feedback from our customer, how the team dealt with
